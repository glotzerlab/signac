...
def info(args):
    ...
    if args.status:
        ...
        if n_pot_dead:
            ...
            if not args.pulse:
                print(
                    "Use 'signac info -p' to check job status and 'signac cleanup' to remove dead jobs from the database.")
...
def view(args):
    ...
    if args.flat:
        if args.url or args.workspace or args.copy or args.script:
            raise ValueError(
                "Illegal combination of arguments: '--flat' can only be combined with prefix argument.")
    ...
    if args.script:
        ...
        if os.path.exists(args.prefix) and os.listdir(args.prefix):
            ...
            return
        project.create_view(url=args.url, make_copy=args.copy,
                            workspace=args.workspace, prefix=args.prefix)
...
def run_checks(args):
    ...
    for msg, check_ in checks:
        ...
        try:
            ...
            print(
                "You can set a different host with 'signac config set database_host $YOURHOST'.")
            if args.verbosity > 0:
                raise
            ...
            print("Your current auth mechanism is set to '{}'. Is that correct?".format(
                auth_mechanism))
            print("Configure the auth mechanism with:")
            print(
                "signac config set database_auth_mechanism [none|SCRAM-SHA-1|SSL-x509]")
            if args.verbosity > 0:
                raise
            encountered_error = True
...
def main(argv=None):
    ...
    parser_cleanup.add_argument(
        '-t', '--tolerance-time',
        type=int,
        help="Tolerated time in seconds since last pulse before a job is declared dead (default={}).".format(
            default_wait),
