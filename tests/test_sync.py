...
class JobMergeTest(BaseJobTest):

    def test_merge_no_implicit_init(self):
    ...
    def test_file_merge_deep(self):
        ...
        with job_src:
            ...
            with open('subdir/test2', 'w') as file:
                file.write('test2')
        self.assertTrue(job_src.isfile('test'))
        job_dst.merge(job_src, deep=True)
        self.assertIn(job_dst, self.project)
        self.assertTrue(job_dst.isfile('test'))
        self.assertTrue(job_dst.isfile('subdir/test2'))
        with open(job_dst.fn('test')) as file:
    ...
    def test_file_merge_with_conflict(self):
        job_dst = self.open_job({'a': 0})
        job_src = self.open_job({'a': 1})
        differs = self._reset_differing_jobs((job_dst, job_src))
        self.assertTrue(differs('test'))
        self.assertTrue(differs('subdir/test2'))
        with self.assertRaises(FileMergeConflict):
            job_dst.merge(job_src)
        job_dst.merge(job_src, sync.FileMerge.never)
        self.assertTrue(differs('test'))
        self.assertTrue(differs('subdir/test2'))
        job_dst.merge(job_src, sync.FileMerge.always, exclude='test')
        self.assertTrue(differs('test'))
        self.assertTrue(differs('subdir/test2'))
        job_dst.merge(job_src, sync.FileMerge.always, exclude=['test', 'bs'])
        self.assertTrue(differs('test'))
        self.assertTrue(differs('subdir/test2'))
        sleep(1.0)
    ...
    def test_file_merge_strategies(self):
        job_dst = self.open_job({'a': 0})
        job_src = self.open_job({'a': 1})
        ...
        self.assertTrue(differs('test'))
        self.assertTrue(differs('subdir/test2'))
        job_dst.merge(job_src, sync.FileMerge.always)
        self.assertFalse(differs('test'))
        self.assertFalse(differs('subdir/test2'))
        reset()
        self.assertTrue(differs('test'))
    ...
    def _reset_document_merge(self):
    ...
    def test_document_merge(self):
        job_dst, job_src = self._reset_document_merge()
        job_dst.merge(job_src)
    ...
    def test_document_merge_nested(self):
        job_dst, job_src = self._reset_document_merge()
        job_dst.document['nested'] = dict(a=0)
        with self.assertRaises(DocumentMergeConflict):
            job_dst.merge(job_src)
        self.assertNotEqual(job_src.document, job_dst.document)

    def test_document_merge_explicit_overwrit(self):
        job_dst, job_src = self._reset_document_merge()
        job_dst.merge(job_src, doc_merge=sync.DocMerge.update)
        self.assertEqual(job_src.document, job_dst.document)

    def test_document_merge_overwrite_specific(self):
        job_dst, job_src = self._reset_document_merge()
        job_dst.merge(job_src, doc_merge=sync.DocMerge.ByKey('nested.a'))
        self.assertEqual(job_src.document, job_dst.document)

    def test_document_merge_partially_differing(self):
        job_dst, job_src = self._reset_document_merge()
        job_dst.document['a'] = 0
        job_dst.merge(job_src)
        self.assertEqual(job_src.document, job_dst.document)

    def test_document_merge_differing_keys(self):
        job_dst, job_src = self._reset_document_merge()
    ...
    def test_document_merge_no_merge(self):
        job_dst, job_src = self._reset_document_merge()
        self.assertTrue(sync.DocMerge.NO_MERGE is False)
        job_dst.merge(job_src, doc_merge=False)
        self.assertNotEqual(job_src.document, job_dst.document)
        self.assertEqual(len(job_dst.document), 0)

    def test_document_merge_dst_has_extra_key(self):
        job_dst, job_src = self._reset_document_merge()
    ...
    def test_document_merge_with_error(self):
        job_dst = self.open_job({'a': 0})
        job_src = self.open_job({'a': 1})
        job_dst.document['a'] = 0
        ...
        with self.assertRaises(RuntimeError):
            job_dst.merge(job_src, doc_merge=raise_error)

    def test_document_merge_with_conflict(self):
        job_dst = self.open_job({'a': 0})
        job_src = self.open_job({'a': 1})
        ...
        with self.assertRaises(DocumentMergeConflict):
            job_dst.merge(job_src)
        self.assertNotEqual(job_dst.document, job_src.document)
        job_dst.merge(job_src, doc_merge=sync.DocMerge.NO_MERGE)
        self.assertNotEqual(job_dst.document, job_src.document)
        ...
        reset()     # only merge a
        job_dst.merge(job_src, doc_merge=sync.DocMerge.ByKey('a'))
        self.assertNotEqual(job_dst.document, job_src.document)
        self.assertNotEqual(job_dst.document['nested'], job_src.document['nested'])
        self.assertEqual(job_dst.document['a'], job_src.document['a'])
        reset()     # only merge nested
        job_dst.merge(job_src, doc_merge=sync.DocMerge.ByKey('nested'))
        self.assertNotEqual(job_dst.document, job_src.document)
        self.assertNotEqual(job_dst.document['a'], job_src.document['a'])
        self.assertEqual(job_dst.document['nested'], job_src.document['nested'])
        reset()
        ...
        self.assertEqual(job_dst.document['a'], job_src.document['a'])
        reset()
        job_dst.merge(job_src, doc_merge=sync.DocMerge.ByKey(lambda key: key.startswith('a')))
        self.assertNotEqual(job_dst.document, job_src.document)
        self.assertNotEqual(job_dst.document['nested'], job_src.document['nested'])
        self.assertEqual(job_dst.document['a'], job_src.document['a'])
        reset()
        job_dst.merge(job_src, doc_merge=sync.DocMerge.ByKey(lambda key: key.startswith('nested')))
        self.assertNotEqual(job_dst.document, job_src.document)
        self.assertNotEqual(job_dst.document['a'], job_src.document['a'])
        self.assertEqual(job_dst.document['nested'], job_src.document['nested'])
        reset()
...
class ProjectMergeTest(unittest.TestCase):
    ...
    def test_src_and_dst_identical(self):
        with self.assertRaises(ValueError):
            self.project_a.merge(self.project_a)

    def test_src_and_dst_empty(self):
        self.project_a.merge(self.project_b)
    ...
    def test_doc_merge(self):
        ...
        with self.assertRaises(DocumentMergeConflict):
            self.project_a.merge(self.project_b)
        self.project_a.merge(self.project_b, doc_merge=sync.DocMerge.ByKey('b'))
    ...
    def test_mixed(self):
        self._setup_mixed()
        with self.assertRaises(SchemaMergeConflict):
            self.project_a.merge(self.project_b)
        self.assertEqual(len(self.project_a), 2)
        self.assertEqual(len(self.project_b), 2)
        self.project_a.merge(self.project_b, check_schema=False)
    ...
    def test_with_conflict_never(self):
        ...
        self.project_a.merge(self.project_b, sync.FileMerge.never)
    ...
    def test_selection(self):
        ...
        with self.assertRaises(FileMergeConflict):
            self.project_a.merge(self.project_b)
        with self.assertRaises(FileMergeConflict):
            self.project_a.merge(self.project_b, selection=self.project_a)
        with self.assertRaises(FileMergeConflict):
            self.project_a.merge(self.project_b, selection=self.project_b)
        self.assertEqual(len(self.project_a.find_jobs({'a': 0})), 1)
        self.assertEqual(len(self.project_b.find_jobs({'a': 0})), 1)
        with self.assertRaises(FileMergeConflict):
            self.project_a.merge(self.project_b, selection=self.project_a.find_jobs({'a': 0}))
        with self.assertRaises(FileMergeConflict):
            self.project_a.merge(self.project_b, selection=self.project_b.find_jobs({'a': 0}))
        with self.assertRaises(FileMergeConflict):
            self.project_a.merge(self.project_b, selection=self.project_a.find_job_ids({'a': 0}))
        with self.assertRaises(FileMergeConflict):
            self.project_a.merge(self.project_b, selection=self.project_b.find_job_ids({'a': 0}))
        f = {'a': {'$ne': 0}}
        self.project_a.merge(self.project_b, selection=self.project_a.find_jobs(f))
        self.project_a.merge(self.project_b, selection=self.project_b.find_jobs(f))
        self.project_a.merge(self.project_b, selection=self.project_a.find_job_ids(f))
        self.project_a.merge(self.project_b, selection=self.project_b.find_job_ids(f))
