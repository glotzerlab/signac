...
class TestNeighborList(TestProject):
    def test_neighbors(self):
        ...
        for a, b in product(a_vals, b_vals):
            ...
            if a == 1:
                assert (
                    this_neighbors["a"][2] == self.project.open_job({"a": 2, "b": b}).id
                )
            elif a == 2:
                assert (
                    this_neighbors["a"][1] == self.project.open_job({"a": 1, "b": b}).id
                )

            # b neighbors
            if b == 3:
                assert (
                    this_neighbors["b"][4] == self.project.open_job({"a": a, "b": 4}).id
                )
            elif b == 4:
                assert (
                    this_neighbors["b"][3] == self.project.open_job({"a": a, "b": 3}).id
                )
                assert (
                    this_neighbors["b"][5] == self.project.open_job({"a": a, "b": 5}).id
                )
            elif b == 5:
                assert (
                    this_neighbors["b"][4] == self.project.open_job({"a": a, "b": 4}).id
    ...
    def test_neighbors_ignore(self):
        ...
        for b in b_vals:
            ...
            if b == 3:
                ...
                assert (
                    this_neighbors["b"][3]
                    == self.project.open_job({"b": 3, "2b": 6}).id
                )
                assert (
                    this_neighbors["b"][5]
                    == self.project.open_job({"b": 5, "2b": 10}).id
                )
            elif b == 5:
                assert (
                    this_neighbors["b"][4]
        ...
        for a in a_vals:
            ...
            if c == 2:
                assert (
                    ...
                    == self.project.open_job({"a": {"c": 3}}).id
                )
            elif c == 3:
                assert (
                    this_neighbors["a.c"][2]
                    == self.project.open_job({"a": {"c": 2}}).id
                )
                assert (
                    this_neighbors["a.c"][4]
                    == self.project.open_job({"a": {"c": 4}}).id
                )
            elif c == 4:
                assert (
                    this_neighbors["a.c"][3]
                ...
                assert (
                    this_neighbors["a.c"]["5"]
                    == self.project.open_job({"a": {"c": "5"}}).id
                )
            elif c == "5":
                assert (
                    this_neighbors["a.c"][4]
                ...
                assert (
                    this_neighbors["a.c"]["hello"]
                    == self.project.open_job({"a": {"c": "hello"}}).id
                )
    ...
    def test_neighbors_disjoint_ignore(self):
        ...
        assert neighbors_project == neighbors_job
        assert (
            neighbors_project["x.n"]["values"]
            == self.project.open_job({"x": {"n": "values"}}).id
        )
