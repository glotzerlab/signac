...
class BasicShellTest(unittest.TestCase):
    ...
    def test_init_project(self):
        ...
        self.call('signac init my_project'.split())
    ...
    def test_init_project_in_project_root(self):
        ...
        with self.assertRaises(ExitCodeError):
            self.call('signac init second_project'.split())

    def test_project_id(self):
        os.chdir(self.tmpdir.name)
        self.call('signac init my_project'.split())
        self.assertEqual(str(signac.get_project()), 'my_project')
        self.assertEqual(self.call('signac project'.split()).strip(), 'my_project')
    ...
    def test_project_workspace(self):
        ...
        self.assertEqual(str(signac.get_project()), 'my_project')
        self.assertEqual(
            self.call('signac project --workspace'.split()).strip(),
    ...
    def test_job_with_argument(self):
        ...
        self.call('signac init my_project'.split())
        self.assertEqual(
            self.call(['signac', 'job', '{"a": 0}']).strip(),
    ...
    def test_job_with_argument_workspace(self):
        ...
        self.assertEqual(
            self.call(['signac', 'job', '--workspace', '{"a": 0}']).strip(), wd_path)
    ...
    def test_job_with_argument_create_workspace(self):
        ...
        self.call(['signac', 'job', '--create', '{"a": 0}'])
