...
class TestBasicShell:
    ...
    def test_view(self):
        ...
        for sp in sps:
            ...
            assert os.path.realpath('view/a/{}/job'.format(sp['a'])) == os.path.realpath(
                project.open_job(sp).workspace()
            )
    ...
    def test_find(self):
        ...
        with pytest.deprecated_call():
            for i in range(3):
                assert (
                    self.call(
                        'python -m signac find --doc-filter'.split() + ['{"a": ' + str(i) + '}']
                    ).strip()
    ...
    def test_import(self):
        ...
        err = self.call(f"python -m signac import {self.tmpdir.name}".split(), error=True)
        ...
        # invalid combination
        with pytest.raises(ExitCodeError):
            self.call(f'python -m signac import {prefix_data} --sync-interactive --move'.split())
    ...
    def test_shell_with_jobs_and_selection(self):
        ...
        out = self.call(
            python_command, 'print(str(project), job, len(list(jobs))); exit()', shell=True
        )
    ...
    def test_config_host(self):
        ...
        self.call('python -m signac config --local host Mongo --remove'.split(), input='y')
        cfg = self.call('python -m signac config --local show'.split())
    ...
    def test_config_verify(self):
        ...
        err = self.call(
            'python -m signac config --local verify'.split(), error=True, raise_error=False
        )
        assert 'Did not find a local configuration file' in err
        self.call('python -m signac init my_project'.split())
        err = self.call('python -m signac config --local verify'.split(), error=True)
